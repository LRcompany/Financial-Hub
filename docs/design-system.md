...
- **Campo de formulário**: `.input`/`.saveBtn` únicos em `styles/cards.module.css` (25/08) — não duplicar por página. Borda 1px + `--r-sm` (borda em input/card é ok, a regra "nunca borda" é só de botão); setinhas nativas de `<input type="number">` removidas (`-webkit-appearance: none` nos spin buttons) — controle de sistema operacional destoa do resto do app, que não usa nenhum.
