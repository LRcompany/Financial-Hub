...
- **Gráfico de barra vertical** (`VerticalBarChart`, 25/08): a maior barra ganha `linear-gradient(180deg, var(--accent), var(--accent-hover))` + rótulo em pill preto — é a **única** exceção documentada à regra "gradiente só em gráfico de linha": aqui o gradiente marca destaque (qual é o maior), não é decoração de fundo. Barras não-destacadas ficam em `var(--accent-soft)`, sólido.
