...
- [ ] Compra "PEOPLE BIKE SHOP" (C6, R$1.159, total=10) não tem projeção automática de parcela futura porque a Pluggy não manda `billForecastDate` pra ela (ver seção "Mais 3 correções pontuais de parcela" acima) — 4 linhas manuais (`description`="PEOPLE BIKE SHOP") seguram o lugar por enquanto. Quando a Pluggy passar a mandar o forecast (deve ser quando a parcela 3 fechar fatura), o sync vai criar as linhas reais com `externalId` — nesse momento apagar as 4 manuais (viram duplicata).
