...
**Pendente pra valer em PRODUÇÃO**: a migration e o código novo precisam ser deployados, e então uma sincronização de verdade do BTG precisa rodar (botão "Sincronizar" em Configurações, ou um sync automático) pra popular o `DividendPayment` de produção pela primeira vez — sem isso, o gráfico de produção continua só com setembro até essa sincronização acontecer.
