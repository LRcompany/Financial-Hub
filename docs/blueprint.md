...
- **Investigação de cripto (Phantom)**: Luiz reportou BTC "faltando" e um token "token.com" a US$69. Reconsulta on-chain (`getTokenAccountsByOwner`, programa antigo **e** Token-2022) confirmou que a carteira Solana informada só tem SOL + Lux Token — nada mais, em nenhum dos dois padrões de token. BTC não é um ativo Solana (endereço Bitcoin tem formato próprio, diferente do endereço Solana que ele mandou) — Phantom trata isso como uma chain separada com endereço próprio; falta o endereço Bitcoin pra sincronizar. "Token.com" existe de verdade (ticker `TOKEN`, mint Solana `Ch2veYHxMWBDgw77nxWvJeGz6YjBD2g9cm21fNriGjGE` via CoinGecko) mas cotado a **US$0,00026**, não US$69, e não está na carteira consultada — reportado a ele pra esclarecer em vez de inventar um ajuste.
