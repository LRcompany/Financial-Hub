...
- [ ] Registro de DAS (Projetos) virar modal em vez de formulário inline.
- [ ] Edição de transação ("Transações do mês" em Orçamento, "Últimas transações" no Dashboard) virar modal — hoje a lista inteira é editável inline (nota, categoria), Luiz quer a lista limpa e um clique abrindo modal com os campos + Salvar.
- [ ] Hover/animação em todo botão clicável do site — hoje só o cursor muda, sem feedback visual nenhum.
