...
- [ ] `pluggyTransactionSync.ts` nunca atualiza uma transação já sincronizada (só verifica se o `externalId` existe e pula) — se a Pluggy corrigir a data/valor de uma transação depois do 1º sync (aconteceu com 1 parcela BTG, ver "Bug real..." acima), a correção nunca chega até fazer um `UPDATE` manual. Cogitado e adiado a pedido do Luiz (caso isolado, auditoria não achou mais nenhum) — considerar se acontecer de novo.
