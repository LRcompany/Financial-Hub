...
### Ícone de estouro reordenado + círculo, sidebar fixa, sino com aviso, modo privacidade (11/09, mesmo dia)

4 pedidos numa tacada só, a partir de 2 screenshots + 1 mockup do Luiz:

1. **`OverBudgetIcon` (`components/Badge.tsx`)** — o `AlertTriangle` solto que existia duplicado em 8 lugares (Dashboard, Orçamento, `CategoryBreakdownModal`, `MonthlyReportModal`) virou um componente único: círculo `--danger-soft`/`--danger`, ícone 14px (tamanho padrão de ícone de interface do design system), **sempre depois** do texto/valor — antes vinha antes (`<AlertTriangle/>R$ 416,74`), pedido do Luiz foi inverter pra `R$ 416,74 <círculo>`. As 8 CSS classes antigas (`.overIcon`/`.progressOverIcon`, 4 arquivos diferentes) ficaram mortas e foram removidas.
2. **Sidebar fixa ao rolar** (`AppLayout.module.css`) — `.sidebar` (desktop, ≥900px) ganhou `position: sticky; top: 0; align-self: flex-start; height: 100svh`. Sem isso, o `.shell` (flex row) esticava a sidebar pra acompanhar a altura de `.main` (que cresce com o conteúdo da página), e ela rolava junto — `align-self: flex-start` tira do stretch padrão do flexbox.
3. **Sino com pontinho vermelho** — reaproveita a MESMA contagem que já alimentava o banner "compra sem categoria" do Início (`GET /transactions/uncategorized-groups`), agora também buscada em `AppLayout.tsx` (dono do header, existe em toda página) e refeita a cada troca de rota (`useLocation().pathname` como dependência — o layout não desmonta entre páginas, sem isso o pontinho ficaria preso depois de categorizar tudo em outra tela).
4. **Modo privacidade** — pedido do Luiz: *"quero mostrar a plataforma pra alguém sem que a pessoa veja meus números de fato... apenas nos valores"*. Ícone de olho no header (substituiu a lupa de busca, que nunca teve ação nenhuma) alterna um `PrivacyContext` (`lib/PrivacyContext.tsx`, persistido em localStorage) que borra (`filter: blur(6px)`) todo valor em R$ da tela via um componente `<Money>` (`components/Money.tsx`) — nunca rótulo, categoria, percentual, data ou quantidade, só o dinheiro. Aplicado em TODO lugar do app que mostra R$: Dashboard, Orçamento, Patrimônio, Projetos, Configurações, todos os modais (relatório mensal, detalhamento de categoria, revisão de transação/orçamento), `SpentPlannedValue` (par gasto/planejado — um fix aqui cobre dezenas de call sites de uma vez), `BalanceChangeBadge`, `VerticalBarChart`/`RankedBarList` (inclusive o detalhe no hover), `DividendsByMonthChart`, `ClientPieChart` (tooltip E legenda — essas usavam `toLocaleString` direto em vez do `currency()` compartilhado, por isso escaparam da primeira varredura por `grep "currency("` e só apareceram na verificação visual) e `SmoothLineChart` (tooltip do hover, usado em todo gráfico de evolução/gasto diário/investido por mês). Não borra `title=` de tooltip nativo do navegador (não são visíveis sem hover deliberado e CSS não alcança esse conteúdo) nem valor dentro de `<input>` sendo editado (meta de patrimônio, aporte mensal). Verificado desktop/mobile/dark mode: toggle liga/desliga instantâneo, persiste ao navegar entre páginas.

### Reordena box de Proventos + hover "de onde veio a grana" (11/09, mesmo dia)

Luiz: *"jogue o box dos proventos para baixo do destaque do mês. Ficará Alocação + Destaque do mês em uma linha e os proventos na outra. Quando eu passar o mouse em proventos, quero saber de onde veio a grana."*

- **Reorder**: em `Patrimonio.tsx`, o card "Proventos recebidos" (`fullWidth`) estava ENTRE "Alocação de investimentos" e "Destaques do mês" — como é full-width, isso quebrava a linha dos outros dois sem necessidade. Movido pra depois dos dois: agora Alocação+Destaques dividem uma linha, Proventos ocupa a linha de baixo sozinho.
- **Hover "de onde veio a grana"**: `wealth.ts` agora calcula, pra cada mês de `dividendsByMonth`, um `breakdown: {label, value}[]` — soma por ativo (ticker, ou nome quando não tem ticker) dentro daquele mês, maior primeiro, só entra quem realmente pagou (>0). Front (`DividendsByMonthChart.tsx`) envolve o rótulo do mês (embaixo da barra) no `HoverCard` genérico do projeto (mesmo padrão já usado em `VerticalBarChart`/`RankedBarList` — nunca um hover novo do zero), listando cada ativo com `HoverRow` + `<Money>`.
- **Achado no meio do caminho**: o popup do `HoverCard` (`.popup`) não tinha `max-height` — testando com uma corretora cheia (BTG, 24 Ação + 18 FII pagando no mesmo mês = 42 linhas), o popup esticava pra fora da tela. Adicionado `max-height: 320px; overflow-y: auto` + `pointer-events: auto` (era `none` — sem isso o popup ficava travado, não dava pra rolar o mouse dentro dele) no componente COMPARTILHADO `HoverCard.module.css`, então todo hover-detalhe do app (não só esse) ganha esse limite de segurança.

Verificado local (dado fake em `dev.db`, restaurado depois): linha Alocação+Destaques lado a lado, Proventos embaixo cheio, hover no mês de agosto mostra as 42 linhas rolando dentro do popup.

### Nova tabela `DividendPayment` — proventos de jan a set, não só a partir de agosto (11/09, mesmo dia)

Luiz, depois de eu reportar que só agosto/setembro tinham provento real: *"me traga todos desse ano, não apenas o de agosto. De janeiro até agora."*

**Causa raiz investigada com dado real** (nunca supor): o gráfico "por mês do ano" só mostrava agosto/setembro preenchidos porque `dividendsByMonth`/`dividendsThisMonth` liam o campo `PositionSnapshot.dividends`, e o BTG só passou a sincronizar Ação/FII por ATIVO INDIVIDUAL via Pluggy a partir de agosto/2026 — de janeiro a julho a carteira inteira vivia agregada num único `PositionSnapshot` manual ("MANUAL:BTG:AÇÕES"/"FIIS", sem ticker). Só que o extrato de transações da Pluggy (`GET /investments/{id}/transactions`) já tinha histórico bem mais antigo — confirmado ao vivo: HTMX11 paga todo mês desde set/2025, VALE3/BBSE3 também têm pagamentos em vários meses de 2026. O backfill retroativo (`fetchAndSyncDividends`) só fazia `updateMany` num `PositionSnapshot` que já existisse pro mês — pra jan-jul isso nunca existia por ticker, então esses meses ficavam pra sempre vazios mesmo com dinheiro real recebido.

**Fix**: nova tabela `DividendPayment` (`brokerId, securityId, month, year, amount`, unique nos 4 primeiros) — grava o provento de TODO mês do extrato incondicionalmente, sem depender de já existir um `PositionSnapshot` daquele mês. `pluggySync.ts` agora faz upsert nela (além do `updateMany` de backfill no `PositionSnapshot`, mantido pra alimentar a coluna por posição quando o mês tem snapshot de verdade). `wealth.ts` (`dividendsByMonth`/`dividendsThisMonth`/`dividendsLastMonth`/`dividendsThisYear`) e `positions.ts` (`dividendsByExactMonth`) passam a ler dessa tabela nova, não mais do campo no snapshot.

**Verificado com sync real contra dev.db** (nunca contra prod — mesma credencial Pluggy real, mas base local): rodei `syncBrokerInvestments` isolado (script padrão, apagado depois) e o `DividendPayment` populou 14 meses de verdade, de ago/2025 a set/2026, com valores reais variando por mês (R$686 a R$2.633) — gráfico em Patrimônio passou a mostrar jan-set/2026 cheio, com breakdown por ativo correto no hover de cada mês (ex: janeiro lista KNCR11 R$332,80, HGRE11 R$189, MXRF11 R$165,20...). `dev.db` restaurado ao estado original depois (a tabela nova ficou vazia de novo, só a estrutura permanece via migration).

**Deployado em produção** (migration + código, mesmo dia): backup do `prod.db` antes da migration (apagado depois de confirmar `PositionSnapshot`/`AppAuth` intactos), `prisma migrate deploy`, `prisma generate` (o client do servidor precisa ser regenerado depois de uma migration nova — esquecido na primeira tentativa de build, corrigido na hora), build, `pm2 restart`. **Ainda falta** uma sincronização de verdade do BTG rodar em produção (botão "Sincronizar" em Configurações) pra popular o `DividendPayment` de produção pela primeira vez — tentei disparar isso eu mesmo via script, bloqueado pelo classificador de ações em produção; pedido ao Luiz pra clicar.

### Lançamento manual de provento — fundo VALORA (11/09, mesmo dia)

Luiz: *"em relação ao fundo Valora, acho importante mostrar os rendimentos que recebo deles... vou adicionar manualmente os valores que recebi desde a criação do fundo, com a data e o valor. Assim ele vai aparecer na coluna de Rendimento."* Perguntei via `AskUserQuestion` dois pontos de escopo antes de implementar: (1) o lançamento manual deve somar no total/gráfico agregado "Proventos recebidos" também, não só na coluna da posição — confirmado que sim; (2) o botão deve valer pra qualquer posição tipo Fundo (não só VALORA especificamente) — confirmado, reaproveitável pra outro fundo parecido no futuro.

- **Backend**: `positions.ts` ganha 3 rotas — `GET /dividend-payments?brokerId&securityId` (histórico da posição), `POST /dividend-payments` (cria/substitui o provento de UM mês — reenviar a mesma data corrige um valor errado, não duplica) e `DELETE /dividend-payments/:id`. Grava direto na `DividendPayment` — a MESMA tabela que a Pluggy usa pra Ação/FII, então o lançamento manual automaticamente conta em tudo que já lê dessa tabela (coluna "Proventos (mês)", card/gráfico agregado, seta de `MonthDelta`), sem nenhum código de exibição separado. Resposta de `/positions` ganhou `brokerId`/`securityId` por posição (só tinha o NOME da corretora antes, não servia pra endereçar um registro no banco).
- **Gráfico "por mês do ano" vira 3 séries**: `wealth.ts` soma `Fundo` como terceira categoria ao lado de Ação/FII (`dividendsByMonth[].fundo`). Novo token `--dividends-fundo` (`#F2934A` claro / `#FFAB6B` escuro — outro stop já existente do mesmo `--grad`, mesmo princípio do `--dividends-fii`). `DividendsByMonthChart` ganha o terceiro segmento empilhado + terceiro item de legenda.
- **Frontend**: botão "+" (ghost, 12px) na coluna "Proventos (mês)" de toda posição tipo Fundo (tabela desktop e card mobile) abre `ManualDividendModal` — form simples (data + valor) + lista dos lançamentos já feitos com opção de apagar. Fica ABERTA depois de salvar (só fecha no X ou clique fora) — pensado pra lançar vários meses seguidos de uma vez ("desde a criação do fundo"), sem reabrir a cada um.
- **Achado no meio do caminho**: o breakdown do hover ("de onde veio a grana") usava `security.ticker ?? security.name` como rótulo — pra Ação/FII isso é o ticker de bolsa (PETR4, HGLG11), mas a Pluggy manda o CNPJ no campo `ticker` de um Fundo (ex: "60.645.828/0001-29"), que apareceu cru no hover em vez do nome "VALORA...". Corrigido pra só usar `ticker` quando o tipo é Ação/FII, mesma regra que `displayName()` já aplicava no front — achado generalizável, qualquer campo que hoje só existe pensando em Ação/FII precisa ser revisto quando outro tipo passa a ter dado real ali.

Verificado local (dev.db, com o VALORA real de produção): botão aparece só em Fundo, modal lista/adiciona/remove lançamento, coluna da posição atualiza na hora (sem reload), card e gráfico agregado somam o valor lançado, hover mostra "VALORA INFRA..." (não mais o CNPJ), dark mode conferido. `DividendPayment` de teste apagado depois (`DELETE FROM DividendPayment`).

**Pendente**: deploy em produção (mesmo fluxo do item acima — nenhuma migration nova, só código).

### Leva de 7 pedidos — Tesouro Direto duplicado, casca única de modal, hover flutuante, cores neutras (11/09, mesmo dia)

Luiz mandou 7 pontos numa mensagem só, com prints. Nem tudo virou código nessa leva — 3 pontos (registro do DAS em modal, edição de transação em modal, hover em todo botão) ficaram pendentes por causa do tamanho da leva; documentado como pendência abaixo.

1. **Tesouro Direto - LFT aparecendo várias vezes**: Luiz lembrava de uma "regra que criamos" pra isso — investigado a fundo (histórico de commits + código), e essa regra NUNCA existiu pra tabela de posições; o que existe (commit `47b61ce`, 08/09) é a dedup do DROPDOWN de "Registrar aporte" (assunto parecido, feature diferente). Causa raiz de verdade: a Pluggy trata cada COMPRA de Renda Fixa como um Investment separado (confirmado ao vivo: mesmo ISIN, `id` diferente por lote), então cada compra do mesmo título virava uma linha nova na tabela. **Fix**: `positions.ts` consolida lotes por corretora+ISIN (nome como fallback quando não tem ISIN) — soma quantidade/investido/valor atual, recalcula preço unitário sobre a quantidade total. Nunca agrupa só por NOME quando existe ISIN (dois títulos diferentes podem ter nome comercial igual com vencimento diferente — confirmado com dado real: duas séries de LFT, ISINs `BRSTNCLF1RU6`/`BRSTNCLF1RL5`, ficam em linhas separadas). Verificado via API real: 10 lotes de LFT (2 ISINs) viraram 2 linhas.
2. **Tooltip de hover quebrado**: `HoverCard` (nome de ativo, "de onde veio a grana"...) tinha o popup como filho ABSOLUTO do próprio gatilho — qualquer ancestral com `overflow-x/y` diferente de `visible` no caminho (ex: `.tableWrap { overflow-x: auto }`, que por regra do CSS também vira `overflow-y: auto`) cortava o popup, que aparecia espremido/sobreposto em cima das linhas da tabela. **Fix**: popup renderiza num PORTAL pra `document.body`, `position: fixed` com coordenadas calculadas do `getBoundingClientRect()` do gatilho no `mouseenter` — escapa de qualquer ancestral com scroll. Show/hide trocou de `:hover` em CSS pra classe aplicada via JS (`.popupVisible`), já que o popup não é mais descendente do gatilho no DOM.
3. **Casca única de modal (`ModalShell`)**: TODA modal do app (9 no total) tinha seu próprio `overlay`/`sheet` copiado, cada um com o MESMO bug — `.sheet { overflow-y: auto }` na sheet inteira, rolando cabeçalho junto (nada fixo). Criado `components/ModalShell.tsx` — cabeçalho (título+subtítulo+ações) e rodapé (Cancelar/Salvar, quando existe) ficam FORA do scroll, só `.content` rola. Suporta `printable` pro caso especial do `MonthlyReportModal` (relatório que vira PDF via `window.print()` — no papel o cabeçalho some e o conteúdo perde o limite de altura pra paginar direito). Migradas: `ContributionModal`, `CategoryBreakdownModal`, `BudgetReviewModal`, `TransactionReviewModal`, `InstallmentReviewModal`, `TransactionModal`, `MonthlyReportModal`, `ManualPositionsModal`, `ManualDividendModal` — nenhuma modal nova pode reimplementar overlay/sheet do zero, sempre `<ModalShell>`.
4. **Fundo de categoria azul → cinza**: `Orcamento.module.css` → `.accordionOpen` (categoria-mãe expandida) usava `--accent-soft` (a mesma cor de "isso está selecionado/ativo") só porque o grupo estava aberto — trocado pra `--fill-muted` (cinza neutro, é só um estado de exibição, não uma seleção de verdade).
5. **Banner "Ultrapassou..." em vermelho**: `.overBanner` (`CategoryBreakdownModal`) usava `--fill-muted`/`--ink-soft` (cinza neutro) — trocado pra `--danger-soft`/`--danger`. Diferente da regra "nunca colorir um VALOR isolado" (`SpentPlannedValue`/`MonthDelta` continuam neutros) — aqui é uma FRASE inteira de aviso, não um número solto, mesma cor semântica do círculo do `OverBudgetIcon` só que aplicada ao texto.

**Pendente (não coube nessa leva, documentado pra próxima)**:
- [x] Registro de DAS (Projetos) virar modal em vez de formulário inline — feito, ver "Os 3 itens que ficaram de fora" abaixo.
- [x] Edição de transação ("Transações do mês" em Orçamento, "Últimas transações" no Dashboard) virar modal — feito, ver "Os 3 itens que ficaram de fora" abaixo.
- [x] Hover/animação em todo botão clicável do site — feito, ver "Os 3 itens que ficaram de fora" abaixo.
...
### Os 3 itens que ficaram de fora da leva de 7 pedidos + design system documentado (11/09, mesmo dia)

Luiz: *"pode fazer os 3 itens que ficaram de fora. E depois disso eu quero que você documente no design system todas as regras de UX... para quando eu gerar um novo sistema, usar esse design system como suporte."*

1. **Registro do DAS em modal**: `NewTaxPaymentForm` (formulário inline em Projetos, sempre visível quando `showNewTax` ligava) virou `NewTaxPaymentModal` — mesmo `<ModalShell>`, botão "+ Registrar DAS" agora abre a modal em vez de alternar um form embutido. **Achado no meio do caminho**: `Projetos.tsx` tinha um componente `Modal` LOCAL (overlay/sheet próprios, nomes `styles.modalOverlay`/`styles.modalSheet` — por isso a varredura original da "casca única de modal" não pegou) usado por MAIS 4 formulários — "Novo projeto", "Novo recebimento", "Novo fornecedor no projeto" e "Pagamento a fornecedor". Migrados os 4 pro `ModalShell` também (cada `footer` ganhou Cancelar/Salvar com o padrão `form="<id>"`) e apagado o `Modal` local morto — nenhuma modal de `Projetos.tsx` ficou de fora da regra "toda modal usa `ModalShell`" (ver design-system.md).
2. **Edição de transação em modal**: criado `components/TransactionEditModal.tsx` — modal única com categoria (`<Select>`, só quando a transação não é transferência/receita) + nota (`<Input>`) + Salvar, usada em AMBOS os lugares que hoje mostram "últimas transações": "Todas as transações do mês" (Orçamento) e "Últimas transações" (Dashboard, que até aqui era 100% só-leitura, com um comentário no código dizendo "edição fica em Orçamento"). Em Orçamento, a lista perdeu o `<Select>`/`<Input>` abertos direto em cada linha (estado `editingNoteId`/`changeTransactionCategory`/etc. removido todo) e virou `cards.listRow` + `cards.listRowButton` só-leitura — clicar em qualquer linha abre a modal com os dados daquela transação. Dashboard ganhou o mesmo tratamento: busca `leafCategories` (`api.transactionLeafCategories()`, endpoint que já existia pro Orçamento) e as linhas de "Últimas transações" viraram clicáveis do mesmo jeito. Salvar dispara `categorizeTransactionGroup`/`updateTransactionNote` (as mesmas chamadas que já existiam) e recarrega a lista de transações de onde foi aberta.
3. **Hover em todo botão clicável do site**: varredura em TODO arquivo `.module.css` com `cursor: pointer` (20 arquivos) procurando seletor sem `:hover` correspondente — a maior parte dos botões do app realmente não tinha nenhum feedback visual além do cursor, confirmando o que o Luiz via de olho. Adicionado hover + `transition: background-color 0.15s ease` em todos, seguindo 4 variações conforme a cor de fundo em repouso (documentado em detalhe no design-system.md, seção "Botões, hovers e animações"): fundo `--accent` → `var(--accent-hover)` (token que já existia em `tokens.css`, nunca tinha sido usado); fundo `--fill-muted` → `color-mix(in srgb, var(--fill-muted) 90%, var(--ink) 10%)` (nunca `filter: brightness()` — descartado cedo por ficar imperceptível num dos dois temas, já que um cinza perto de branco no claro e perto de preto no escuro mal muda de tom com brightness); fundo transparente/ghost → ganha `--fill-muted`; fundo `--danger-soft` → intensifica com `color-mix` puxando mais `--danger`. `IconButton.module.css` (base de todo botão de ícone) ganhou `transition` + `transform: scale(0.96)` no `:active` como fundação compartilhada; `.fab` (botão flutuante) segue o mesmo princípio. Limpeza de tabela: `noteButton`/`noteInput`/`transactionCategorySelect`/`transactionStaticLabel` (CSS da antiga edição inline em Orçamento, item 2 acima) ficaram mortos e foram removidos de `Orcamento.module.css`.
4. **`docs/design-system.md` reescrito por TEMA** (não mais só uma lista cronológica de "achado real") — pedido explícito do Luiz pra virar referência de apoio na hora de gerar um sistema novo. Reorganizado em seções (Cor, Tipografia, Espaçamento, Ícones, Botões/hovers/animações, Campos de formulário, Modais, Listas/tabelas desktop↔mobile, Gráficos, Popup/tooltip, Modo privacidade, Layout, Regras técnicas) — todo fato que já existia foi preservado, só reagrupado por tema; a seção de "Botões, hovers e animações" é nova, junto com a expansão do contrato completo do `ModalShell` (props, padrão `form="id"` no footer, onde procurar modal duplicada) e do padrão de linha clicável (`.listRowButton`/`.progressRowButton`/`.categoryRowButton`, quando cada um se aplica).
